...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, x_mask: torch.Tensor):
        ...
        return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask, key_padding_mask=x_mask)

    def forward(self, x: torch.Tensor, x_mask: torch.Tensor = None, hook=False):
        o = self.attention(self.ln_1(x), x_mask)
        x = x + o[0]
        x = x + self.mlp(self.ln_2(x))
        if hook is True:
            self.attention_map = o[1]
        return x
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, x_mask: torch.Tensor = None):
        for i, block in enumerate(self.resblocks):
            x = block(x, x_mask, i + 1 == len(self.resblocks))
        return x
