...
from model.former_clsvqa import Former_Clsvqa
...
def main(args):
    ...
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=args.eval_batch_size,
                                              num_workers=args.num_workers,
                                              pin_memory=True)
    test_loader_d = torch.utils.data.DataLoader(test_dataset, batch_size=args.eval_batch_size, sampler=sampler_test,
                                              num_workers=args.num_workers,
                                              pin_memory=True)
    ...
    if args.classifier_vqa:
        model = Former_Clsvqa(img_size=args.img_size, vit_type=args.vit_type, freeze_vit=args.freeze_vit,
    ...
    if args.evaluate:
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            if epoch >= args.epochs - 3:
                train(model, test_loader_d, optimizer, epoch, device, args)
